...
const find_next_interval = function(intervals) {
  const result = [];
  const startHeap = new Heap([], null, ((a, b) => b[0] - a[0]));
  const endHeap = new Heap([], null, ((a, b) => b[0] - a[0]));

  intervals.forEach((el, i) => {
    startHeap.push([el.start, i]);
    endHeap.push([el.end, i]);
  });

  for (let i = 0; i < intervals.length; i++) {
    const currentInterval = endHeap.pop();
    while (startHeap.length && currentInterval[0] > startHeap.peek()[0]) {
      startHeap.pop();
    }
    !startHeap.length ? result.push(-1) : result.push(startHeap.pop()[1]);
  }
  return result;
};
