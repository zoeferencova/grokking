...
const find_employee_free_time = function(schedule) {
    ...
    for (let i = 0; i < schedule.length; i++) {
        let employee = schedule[i];
        employee.forEach(window => {
            flatArray.push(window)
        });
    }
    
    flatArray.sort((a, b) => a.start - b.start);

    for (let i = 0; i < flatArray.length-1; i++) {
        if (flatArray[i].end < flatArray[i + 1].start) {
            resultArray.push(new Interval(flatArray[i].end, flatArray[i+1].start));
        }
    }
